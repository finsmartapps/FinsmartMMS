...
export default function ContactDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const loadContact = useCallback(async () => {
    const res = await fetch(`/api/telecaller/contacts/${id}`)
    const d = await res.json()
    if (!res.ok) { router.push('/sales/telecaller/contacts'); return }
  ...
  const loadTimeline = useCallback(async () => {
    const res = await fetch(`/api/telecaller/contacts/${id}/timeline`)
    const d = await res.json()
    if (res.ok) {
  ...
  async function handleGenerateMessage() {
    ...
    const res = await fetch(`/api/telecaller/contacts/${id}/message`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) })
    const d = await res.json()
    if (res.ok) setContact(c => c ? { ...c, generated_message: d.message } : c)
  ...
  async function handleSaveNote() {
    ...
    setSavingNote(true)
    const res = await fetch(`/api/telecaller/contacts/${id}/notes`, {
  ...
  async function handleSaveCall() {
    setSavingCall(true)
    const res = await fetch(`/api/telecaller/contacts/${id}/calls`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
  ...
  async function handleSaveTask() {
    ...
    setSavingTask(true)
    const res = await fetch(`/api/telecaller/contacts/${id}/tasks`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
  ...
  async function handleToggleTask(taskId: string, current: 'pending' | 'completed') {
    ...
    const res = await fetch(`/api/telecaller/contacts/${id}/tasks/${taskId}`, {
  ...
  async function handleDeleteCall(callId: string) {
    await fetch(`/api/telecaller/contacts/${id}/calls/${callId}`, { method: 'DELETE' })
    setCalls(prev => prev.filter(c => c.id !== callId))
  }

  async function handleDeleteTask(taskId: string) {
    await fetch(`/api/telecaller/contacts/${id}/tasks/${taskId}`, { method: 'DELETE' })
    ...
    <div className="flex h-full min-h-0" style={{ height: 'calc(100vh - 56px)' }}>
      ...
      <div className="w-72 flex-shrink-0 border-r border-[#E5E5EA] bg-white overflow-y-auto flex flex-col">
        ...
        <div className="px-4 py-3 border-b border-[#F2F2F7] flex items-center justify-between flex-shrink-0">
          <Link href="/sales/telecaller/contacts"
        ...
        <div className="px-4 py-3 border-b border-[#F2F2F7] flex-shrink-0">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <Link href={`/sales/telecaller/followups?contact=${id}`}
              className="flex flex-col items-center gap-1 group cursor-pointer">
              <div className="w-9 h-9 rounded-full border border-[#E5E5EA] flex items-center justify-center group-hover:border-[#DC2626] group-hover:bg-red-50 transition">
            ...
            <Link href={`/sales/telecaller/meetings?contact=${id}`}
              className="flex flex-col items-center gap-1 group cursor-pointer">
              <div className="w-9 h-9 rounded-full border border-[#E5E5EA] flex items-center justify-center group-hover:border-[#DC2626] group-hover:bg-red-50 transition">
        ...
        {searchFiltered.length === 0 ? (
          ...
          {activeTab === 'intelligence' && (
            <div className="max-w-3xl space-y-5">
              ...
              {/* Generated message */}
              <div className="bg-white rounded-xl border border-[#E5E5EA]" style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
                <div className="px-5 py-3 border-b border-[#F2F2F7] flex items-center justify-between">
                  <p className="text-[13px] font-semibold text-[#1D1D1F]">AI Message</p>
                  <button onClick={handleGenerateMessage} disabled={generatingMsg}
                    className="flex items-center gap-1.5 text-[12px] text-[#DC2626] hover:text-[#C91C1C] font-medium transition disabled:opacity-50">
                    {generatingMsg ? <Loader2 size={12} className="animate-spin" /> : <RefreshCw size={12} />}
                    {contact.generated_message ? 'Regenerate' : 'Generate'}
                  </button>
                </div>
                <div className="px-5 py-4">
                  {contact.generated_message ? (
                    <div className="relative">
                      <p className="text-[13px] text-[#1D1D1F] leading-relaxed whitespace-pre-wrap">{contact.generated_message}</p>
                      <button onClick={() => copyToClipboard(contact.generated_message!, 'msg')}
                        className="mt-3 flex items-center gap-1.5 text-[12px] text-[#AEAEB2] hover:text-[#1D1D1F] transition">
                        {copied === 'msg' ? <><Check size={12} className="text-[#34C759]" /> Copied!</> : <><Copy size={12} /> Copy message</>}
                      </button>
                    </div>
                  ) : (
                    <div className="text-center py-6">
                      <MessageSquare size={20} className="text-[#AEAEB2] mx-auto mb-2" />
                      <p className="text-[12px] text-[#AEAEB2]">Click Generate to create a personalized outreach message</p>
                    </div>
                  )}
                </div>
              </div>
      ...
      <div className="w-64 flex-shrink-0 border-l border-[#E5E5EA] bg-white overflow-y-auto">
        ...
        <div className="px-4 py-4 border-b border-[#F2F2F7]">
          <div className="flex items-center justify-between mb-3">
            ...
            <Link href="/sales/telecaller/followups" className="text-[11px] text-[#DC2626] hover:underline">View all</Link>
          </div>
          {followups.length === 0 ? (
        ...
        <div className="px-4 py-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <Link href="/sales/telecaller/meetings" className="text-[11px] text-[#DC2626] hover:underline">View all</Link>
          </div>
          {meetings.length === 0 ? (
