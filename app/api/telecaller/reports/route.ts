...
export async function GET(req: NextRequest) {
  ...
  const { data: selfProfile } = await supabase.from('profiles').select('role, has_sales, name, email').eq('id', user.id).single()
  if (!selfProfile?.has_sales || selfProfile.role !== 'telecaller') return NextResponse.json({ error: 'Forbidden' }, { status: 403 })
  ...
  const [activitiesRes, logsRes, meetingsRes, holidaysRes, targetsRes, followupsRes] = await Promise.all([
  ...
  return NextResponse.json({
    ...
    summary: { totalCalls, callTarget, totalTarget, achievementPct, daysSubmitted, submissionRate, meetings: meetings.length, followupsDone, followupsTotal: followups.length },
    dailyChart,
    submittedLogs,
    meetings,
    activities,
